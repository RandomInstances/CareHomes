...
function Detail({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="border-t border-[--ch-line] pt-4">
      <h2 className="text-[13px] uppercase tracking-wide text-[--ch-muted] font-semibold mb-1.5">
        {label}
      </h2>
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-3xl px-5 py-8 flex-1 w-full">
        ...
        <nav className="text-sm text-[--ch-muted] mb-4">
          <Link href="/" className="hover:text-[--ch-ink]">Colombo</Link>
          <span className="mx-1.5">/</span>
          <Link href={`/${home.suburb.slug}`} className="hover:text-[--ch-ink]">
            {home.suburb.name}
          </Link>
        </nav>
        ...
        <header className="space-y-2">
          {home.tier === "VERIFIED" ? (
            <span className="inline-flex items-center gap-1.5 bg-[--ch-teal-soft] text-[--ch-teal] rounded-full px-3 py-1 text-xs font-bold">
            ...
            <span className="inline-block bg-[--ch-turmeric-soft] text-[--ch-turmeric] rounded-full px-3 py-1 text-xs font-bold">
              Not yet visited by our team
            </span>
          )}

          <h1 className="text-3xl font-semibold">{home.name}</h1>
          <p className="text-[--ch-ink-2]">
            {home.suburb.name}
        ...
        {wa ? (
          <div className="mt-6 flex flex-wrap gap-3">
            <a
              ...
              target="_blank"
              rel="noopener"
              className="inline-flex items-center gap-2 rounded-full bg-[--ch-teal] text-white font-semibold px-5 py-3"
            >
            ...
            {home.phone ? (
              <a
                href={`tel:${home.phone}`}
                className="inline-flex items-center gap-2 rounded-full border border-[--ch-line-2] bg-[--ch-surface] font-semibold px-5 py-3"
              >
                Call {home.phone}
              </a>
            ) : null}
        ...
        <div className="mt-8 grid gap-5 sm:grid-cols-2">
          {fee ? (
            <Detail label="Monthly fee">
              ...
              {home.feeExcludes.length ? (
                <p className="text-sm text-[--ch-ink-2] mt-1.5">
          ...
          <Detail label="Availability">
            ...
            {home.roomTypes.length ? (
              <p className="text-sm text-[--ch-ink-2] mt-1.5">Rooms: {home.roomTypes.join(", ")}.</p>
            ) : null}
          </Detail>
          ...
          <div className="mt-5">
            <Detail label="Facilities">
              <ul className="flex flex-wrap gap-2 mt-1">
                {home.features.map((f) => (
                  <li key={f} className="rounded-full bg-[--ch-surface] border border-[--ch-line] px-3 py-1 text-sm">
                    {f}
                  </li>
                ))}
              </ul>
        ...
        <div className="mt-8 text-sm text-[--ch-ink-2] bg-[--ch-surface] border border-[--ch-line] rounded-xl p-4">
          {home.tier === "VERIFIED" ? (
            <p>
